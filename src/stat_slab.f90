...
real, dimension(:), pointer :: u1, v1, t1, q1, l1, th1, b1, qv1, N2, ut, vt, tt, qt, &
                               ...
                               u2, v2, w2, t2, q2, l2, uw, vw, tw, qw, lw, tb, qb, tq, b2, bw, th2, qv2, thqv, &
                               uDpDx, vDpDy, wDp, bDp, tDp, qDp, u2t, v2t, w2t, t2t, q2t, uwt, vwt, twt, qwt, tqt, &
                               u2_s, v2_s, w2_s, t2_s, q2_s, l2_s, uw_s, vw_s, tw_s, qw_s, lw_s, tb_s, qb_s, tq_s, b2_s, &
                               bw_s, th2_s, qv2_s, thqv_s, uDpDx_s, vDpDy_s, wDp_s, bDp_s, tDp_s, qDp_s, &
...
contains
  ...
  subroutine init_stat_slab
    ...
    call create_stat_var(ncid, 'uwt', zm_name, my_nf90_real)
    call create_stat_var(ncid, 'vwt', zm_name, my_nf90_real)
    call create_stat_var(ncid, 'twt', zm_name, my_nf90_real)

    call create_stat_var(ncid, 'bw',    zm_name, my_nf90_real)
    call create_stat_var(ncid, 'tb',    zm_name, my_nf90_real)
    call create_stat_var(ncid, 'b2',    zm_name, my_nf90_real)
    call create_stat_var(ncid, 'uDpDx', zm_name, my_nf90_real)
    call create_stat_var(ncid, 'vDpDy', zm_name, my_nf90_real)
    ...
    if (level >= 1) then
       ...
       call create_stat_var(ncid, 'qb',   zm_name, my_nf90_real)
  ...
  subroutine stat_slab
    ...
    do i = 3,nxp-2
       do k = 2,nzp-1
          ...
          uwt_s(k) = uwt_s(k) + a_wp(k,i,j)*(0.25*(a_ut(k,i,j)+a_ut(kp1,i,j)+a_ut(k,ip1,j)+a_ut(kp1,ip1,j)) - 0.5*(ut_s(k) + ut_s(kp1))) &
                              + a_wt(k,i,j)*(0.25*(a_up(k,i,j)+a_up(kp1,i,j)+a_up(k,ip1,j)+a_up(kp1,ip1,j)) - 0.5*(u1_s(k) + u1_s(kp1))) 
          vwt_s(k) = vwt_s(k) + a_wp(k,i,j)*(0.25*(a_vt(k,i,j)+a_vt(kp1,i,j)+a_vt(k,i,jp1)+a_vt(kp1,i,jp1)) - 0.5*(vt_s(k) + vt_s(kp1))) &
                              + a_wt(k,i,j)*(0.25*(a_up(k,i,j)+a_up(kp1,i,j)+a_up(k,i,jp1)+a_up(kp1,i,jp1)) - 0.5*(v1_s(k) + v1_s(kp1))) 
          ...
          tb_s(k)    = tb_s(k)    + (0.5*(a_tp(k,i,j) + a_tp(kp1,i,j)) - 0.5*(t1_s(k) + t1_s(kp1)))&
                                  * (0.5*(a_b(k,i,j) + a_b(kp1,i,j)) - b1_s(k))
          b2_s(k)    = b2_s(k)    + (0.5*(a_b(k,i,j) + a_b(kp1,i,j)) - b1_s(k))**2.
          uDpDx_s(k) = uDpDx_s(k) + a_up(k,i,j)*th00*(a_pexnr(k,ip1,j) - a_pexnr(k,i,j))/dxi
          vDpDy_s(k) = vDpDy_s(k) + a_vp(k,i,j)*th00*(a_pexnr(k,i,jp1) - a_pexnr(k,i,j))/dyi
          wDp_s(k)   = wDp_s(k)   + a_wp(k,i,j)*th00*(a_pexnr(kp1,i,j) - a_pexnr(k,i,j))/(zt(kp1) - zt(k))
          tDp_s(k)   = tDp_s(k)   + (0.5*(a_tp(k,i,j) + a_tp(kp1,i,j)) - 0.5*(t1_s(k) + t1_s(kp1)))&
                                  * th00*(a_pexnr(kp1,i,j) - a_pexnr(k,i,j))/(zt(kp1) - zt(k))
          bDp_s(k)   = bDp_s(k)   + (0.5*(a_b(k,i,j) + a_b(kp1,i,j)) - b1_s(k)) &
                                  * th00*(a_pexnr(kp1,i,j) - a_pexnr(k,i,j))/(zt(kp1) - zt(k))
          ...
          if (level >= 1) then
             ...
             qb_s(k)   = qb_s(k)   + (0.5*(a_rp(k,i,j) + a_rp(kp1,i,j)) - 0.5*(q1_s(k) + q1_s(kp1)))&
                                   * (0.5*(a_b(k,i,j) + a_b(kp1,i,j)) - b1_s(k))
             qDp_s(k)  = qDp_s(k)  + (0.5*(a_rp(k,i,j) + a_rp(kp1,i,j)) - 0.5*(q1_s(k) + q1_s(kp1)))&
                                   * th00*(a_pexnr(kp1,i,j) - a_pexnr(k,i,j))/(zt(kp1) - zt(k))
    ...
    uwt(:) = f_local*uwt_s(:) + (1. - f_local)*uwt(:)
    vwt(:) = f_local*vwt_s(:) + (1. - f_local)*vwt(:)
    twt(:) = f_local*twt_s(:) + (1. - f_local)*twt(:)

    bw(:)    = f_local*bw_s(:)    + (1. - f_local)*bw(:)
    tb(:)    = f_local*tb_s(:)    + (1. - f_local)*tb(:)  + f_local*(1. - f_local)*(t1_s(:) - t1(:))*(b1_s(:) - b1(:)) ! grid?
    b2(:)    = f_local*b2_s(:)    + (1. - f_local)*b2(:)  + f_local*(1. - f_local)*(b1_s(:) - b1(:))**2.
    uDpDx(:) = f_local*uDpDx_s(:) + (1. - f_local)*uDpDx(:)
    vDpDy(:) = f_local*vDpDy_s(:) + (1. - f_local)*vDpDy(:)
    ...
    if (level >= 1) then
       ...
       qb(:)   = f_local*qb_s(:)   + (1. - f_local)*qb(:)   + f_local*(1. - f_local)*(q1_s(:) - q1(:))*(b1_s(:) - b1(:)) ! grid?
  ...
  subroutine write_stat_slab(time, fsttm, nsmp)
    ...
    call write_stat_var(ncid, 'tb',    tb,      irec)
    call write_stat_var(ncid, 'b2',    b2,      irec)
    call write_stat_var(ncid, 'uDpDx', uDpDx,   irec)
    call write_stat_var(ncid, 'vDpDy', vDpDy,   irec)
    ...
    if (level >= 1) then
       ...
       call write_stat_var(ncid, 'qb',   qb,   irec)
