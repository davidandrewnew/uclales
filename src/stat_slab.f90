...
real, dimension(:), pointer :: u1, v1, t1, q1, l1, &
                               u1_s, v1_s, t1_s, q1_s, l1_s, &
                               u2, v2, w2, t2, q2, l2, tw, qw, lw, tq, &
                               u2_s, v2_s, w2_s, t2_s, q2_s, l2_s, tw_s, qw_s, lw_s, tq_s 
...
contains
  ...
  subroutine init_stat_slab
    ...
    v1_s => x1_s(:,2)
    t1_s => x1_s(:,3)
    if (level >= 1) q1_s => x1_s(:,4)
    if (level >= 2) l1_s => x1_s(:,5)
  ...
  subroutine stat_slab
    use grid, only        : a_up, a_vp, a_wp, a_tp, a_rp, liquid, nzp, nyp, nxp, level
    use modutil_mpi, only : par_sum
    implicit none
    ...
    do i = 3,nxp-2
       do k = 2,nzp-1
          ...
          if (level >= 1) then
             q2_s(k) = q2_s(k) + (a_rp(k,i,j) - q1(k))**2.
